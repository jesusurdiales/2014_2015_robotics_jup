...
void MyRobot::following_objetive_state(double ds_values[], double compass_angle)
{
    ...
    if (ds_values[0] > (6 * MAX_DISTANCE) || ds_values[15] > (6 * MAX_DISTANCE))
    {
        // The nearest wall is on the left
    ...
    if (ds_values[3] > (7 * MAX_DISTANCE))
    {
        ...
        cout << "Avoiding too close left wall" << endl;
        return;
    }
    ...
    {
        _mov_mode = TURN_LEFT;
        cout << "Avoiding too close right wall" << endl;
        return;
    }

    // The robot follows compass direction
    cout <<  "I am going straight to my objetive" << endl;
...
void MyRobot::following_right_wall_state(double ds_values[])
{
    ...
    if ((ds_values[1] > MAX_DISTANCE) || (ds_values[14] > MAX_DISTANCE))
    {
        if (ds_values[7] > (8 * MAX_DISTANCE) || ds_values[8] > (8 * MAX_DISTANCE))
        {
            _mov_mode = TURN_LEFT;
    ...
    if (ds_values[13] > MAX_DISTANCE || ds_values[12] > (6 * MAX_DISTANCE))
    {
        _mov_mode = TURN_LEFT;
        cout << "Turning left." << endl;
        return;
    ...
    // The robot knows when the obstacle has been exceeding
...
void MyRobot::following_left_wall_state(double ds_values[])
{
    ...
    // With these conditions the robot avoids crashing with corners
    if (ds_values[2] > MAX_DISTANCE || ds_values[3] > (6 * MAX_DISTANCE))
    {
        _mov_mode = TURN_RIGHT;
        cout << "Turning right." << endl;
        return;
